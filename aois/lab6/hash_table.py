...
class HashTable:
    ...
    def insert(self, key: str, data: str) -> bool:
        ...
        if self.search(key) is not None:
            print(f"Ключ '{key}' уже существует в таблице. Запись отклонена.")
            return False
    ...
    def delete(self, key: str) -> bool:
        ...
        if row.T == 1 and (row.P0 == -1 or row.P0 == h):
            row.U = 0
            print(f"Удалена одиночная запись '{key}' (строка {idx})")
        elif row.T == 1:
            prev_idx = self._find_prev_in_chain(h, idx)
            if prev_idx != -1:
                self.table[prev_idx].T = 1
            row.U = 0
            print(f"Удалена конечная запись цепочки '{key}' (строка {idx})")
        elif row.T == 0 and row.P0 != h:
            next_idx = row.P0
            self._copy_row(next_idx, idx)
            self.table[next_idx].U = 0
            self.occupied_count -= 1
            print(f"Удалена средняя запись '{key}' (строка {idx}, смещение из {next_idx})")
        elif row.T == 0 and row.C == 1:
            next_idx = row.P0
            self._copy_row(next_idx, idx)
            self.table[next_idx].U = 0
            self.occupied_count -= 1
            print(f"Удалена первая запись цепочки '{key}' (строка {idx}, смещение из {next_idx})")
...
if __name__ == "__main__":
    ...
    mountains = [
        ("Эльбрус", "5642"), ("Казбек", "5047"), ("Белуха", "4506"),("Эльбру", "5665"),("Эльбус", "5690"),
